...
type Config struct {
	...
	SineDuration, TabletsPerShard, TargetShardOps, LogInterval, KeyCount, ConcurrencyLimit         int
	SineAmplitude, ReadRatio                                                                       float64
}

type ScalingPhase int

const (
	Baseline ScalingPhase = iota
	PreScale
	Scaling
	PostScale
	Peak
...
type ClusterState struct {
	Nodes, Shards, MinRackShards, DecommissioningNodes int
	Balanced                                           bool
}
...
func parseFlags() Config {
	...
	flag.Float64Var(&c.ReadRatio, "read-ratio", 0.0, "Read Ratio (0.0 - 1.0). Default 0.0")
	flag.IntVar(&c.ConcurrencyLimit, "concurrency", 0, "Concurrency limit (default 8 * numCPU)")
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  -read-ratio float\n")
		fmt.Fprintf(os.Stderr, "        Read Ratio (0.0 - 1.0). Default 0.0\n")
		fmt.Fprintf(os.Stderr, "  -concurrency int\n")
		fmt.Fprintf(os.Stderr, "        Concurrency limit (default 8 * numCPU)\n")
	...
	if c.Token == "" {
		c.Token = os.Getenv("SCYLLA_CLOUD_TOKEN")
	}
	if c.Password == "" {
		if p := os.Getenv("SCYLLA_PASSWORD"); p != "" {
			c.Password = p
		} else {
			c.Password = "scylla"
		}
	...
	if c.ConcurrencyLimit == 0 {
		c.ConcurrencyLimit = 8 * runtime.NumCPU()
	}
	return c
}
...
func (app *App) initDB() {
	...
	if err := session.Query(ksStmt).Exec(); err != nil {
		log.Fatalf("Keyspace creation failed: %v", err)
	}
	...
	if err := session.Query(tblStmt).Exec(); err != nil {
	...
	if err != nil {
		log.Fatalf("App session failed: %v", err)
	}
}
...
func (app *App) initCloudAPI() {
	...
	var clusterResp struct {
		Data struct {
			Cluster struct{ Dc struct{ Id, InstanceId int } }
		}
	}
...
func (app *App) getClusterState() ClusterState {
	...
	var shardCount int
	var rack string
	for iter.Scan(&shardCount, &rack) {
		rackShards[rack] += shardCount
	}
	iter.Close()
	for _, count := range rackShards {
		if count < minShards {
			minShards = count
		}
	}
	if minShards == math.MaxInt {
		minShards = 0
	}
...
func (app *App) generateScalingTraffic() int {
	...
	case Baseline:
		return app.currentBaseline
	case PreScale:
		return ramp(app.currentBaseline, app.currentPeak)
	case Scaling:
		return app.currentPeak
	case PostScale:
		return ramp(app.postScaleStartTraffic, app.config.TargetPeakOps)
	case Peak:
		return app.config.TargetPeakOps
	case ScaleDown:
		return ramp(app.config.TargetPeakOps, app.originalShards*app.config.TargetShardOps)
	case Decommission:
		return app.currentBaseline
	default:
		return app.currentBaseline
...
func (app *App) sendRead() {
	keyId := atomic.AddUint64(&app.keyCounter, 1) % uint64(app.config.KeyCount)
	key := make([]byte, 16)
	binary.LittleEndian.PutUint64(key, keyId)
	binary.LittleEndian.PutUint64(key[8:], keyId)

	query := fmt.Sprintf("SELECT val FROM %s.%s WHERE key = ?", app.config.KeyspaceName, app.config.TableName)
	iter := app.session.Query(query, key).Iter()
	var val []byte
	for iter.Scan(&val) {
	}
...
func (app *App) generateWrites(rps int) {
	...
	sem := make(chan struct{}, app.config.ConcurrencyLimit)
	for i := 0; i < workers; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			numReads := int(float64(batchSize) * app.config.ReadRatio)
			// numWrites := batchSize - numReads
